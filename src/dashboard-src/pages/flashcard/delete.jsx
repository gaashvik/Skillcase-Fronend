...
export default function DeleteFlashSet() {
  ...
  const fetchChapters = async () => {
    ...
    try {
      const res = await api.get(`/admin/getChapters/${proficiency}`);

      // Normalize response: assume array of objects with set_name
  ...
  const handleDelete = async (chapterName) => {
    ...
    try {
      await api.post("/admin/deleteFlashSet", {
        set_name: chapterName,
        proficiency_level: proficiency,
      });

      setStatus(`success:"${chapterName}" deleted successfully!`);
      setChapters((prev) => prev.filter((ch) => ch.set_name !== chapterName));
              ...
              <table className="table-auto w-full">
                ...
                <tbody className="text-sm divide-y divide-gray-200 ">
                  {chapters.map((chapter) => (
                    <tr 
                      key={chapter.set_id || chapter.set_name}
                    ...
                    >
                      ...
                      <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap text-right">
                        <button
                          onClick={() => handleDelete(chapter.set_name)}
                          disabled={deletingChapter === chapter.set_name}
                        ...
                        >
                          {deletingChapter === chapter.set_name ? (
                            <>
        ...
        <div className="flex gap-3">
          ...
          <div className="text-sm text-red-700 ">
            ...
            <p className="text-red-600 ">
              Deleting a chapter will permanently remove all associated flashcard data. This action cannot be undone.
            </p>
          </div>
        </div>
      </div>
