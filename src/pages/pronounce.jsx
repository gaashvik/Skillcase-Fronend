  ...
  return (
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-cyan-50 to-slate-50">
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-8 md:py-12">
        ...
        <div className="relative w-full h-3 bg-slate-100 rounded-full mb-8">
          <div className="h-full bg-gradient-to-r from-slate-700 to-slate-800 rounded-full transition-all duration-300" style={{ width: `${Progress}%` }} />
        </div>
        ...
        >
          <div className="h-full bg-white rounded-3xl shadow-2xl p-12 flex flex-col items-center justify-center border-4 border-slate-500 relative overflow-hidden">
            <button
              ...
              className={`absolute top-6 right-6 p-3 rounded-full shadow-lg transition-all z-20 ${isSpeaking ? 'bg-cyan-600 text-white animate-pulse' : 'bg-white text-cyan-600 hover:bg-cyan-50'}`}
        ...
        <div className="flex items-center justify-center gap-4 mt-8">
          <button onClick={handlePrevious} disabled={currentCard === 0} className="p-3 rounded-xl bg-white text-slate-700 hover:bg-slate-50 shadow-lg">
            <ChevronLeft className="w-6 h-6" />
          </button>
          <button onClick={handleNext} disabled={currentCard === totalCards - 1} className="p-3 rounded-xl bg-white text-slate-700 hover:bg-slate-50 shadow-lg">
    ...
    {isOverlayOpen && selectedTest && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 transition-opacity duration-200">
          <VoiceRecorder card={selectedTest} closeOverlay={closeOverlay}/>
        </div>
      )}
    </section>
  );
