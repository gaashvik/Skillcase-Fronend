...
export default function DeleteInterview() {
  ...
  const handleDelete = async (item) => {
    if (!window.confirm(`Are you sure you want to delete "${item.difficulty}"?`)) return;

    setLoading(true);
    setStatus(`Deleting "${item.difficulty}"...`);
    ...
    try {
      ...
      setStatus(`"${item.difficulty}" deleted successfully!`);
      setChapters((prev) => prev.filter((ch) => ch.difficulty !== item.difficulty));
    } catch (err) {
      console.error(err);
      setStatus(`Failed to delete "${item.difficulty}".`);
    } finally {
      setLoading(false);
    }
  };
              ...
              <div className="mt-6">
                ...
                <div className="space-y-3">
                  {chapters.map((chapter) => (
                    <div
                      key={chapter.interview_id || chapter.difficulty}
                      className="flex items-center justify-between p-4 bg-slate-50 rounded-lg shadow-sm"
                    >
                      <p className="font-medium text-slate-800">{chapter.difficulty}</p>
                      <button
